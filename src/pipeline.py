...
async def run_adk_pipeline(query: str, event_queue: queue.Queue, api_key: str = None, chat_history: list = None):
    """
    ...
    try:
        # We pass history in the input dictionary for the agent to handle
        input_payload = {
            "query": query,
        ...
        response = await runner.run_debug(payload_str)
...
def run_system_generator(query: str, api_key: str = None, chat_history: list = None):
    """
    ...
    def target():
        try:
            ...
            loop.run_until_complete(run_adk_pipeline(query, event_queue, api_key, chat_history))
            loop.close()
        except Exception as e:
            event_queue.put({
                "step": "ERROR",
