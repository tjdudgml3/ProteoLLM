...
PROMPTS = {
    "router_system": """
    You are a Router Agent. Classify input as 'experimenter', 'analyst', or 'internet'.
    - 'experimenter': Wet lab related, protocol, biology experiment.
    - 'analyst': Bioinformatics analysis, pipeline design, data processing.
    - 'internet': General follow-up questions, latest news, information not likely in specific research papers, or if user asks to search the web.
    ...
    """,
    "exp_advisor_system": "You are an Experimental Advisor (Wet Lab). Provide advice with citations. USE GIVEN SOURCE DATA AND REFERENCE ID",
    "analyst_advisor_system": "You are an Analyst Advisor (Bioinformatics). Provide analysis plans with citations. USE GIVEN SOURCE DATA AND REFERENCE ID",
    ...
    "internet_advisor_system": "You are an Internet Research Advisor. Provide comprehensive answers based on web search results. Cite your sources.",
    "summarize_system": f"""
    ...
    Task:
    Provide a detailed answer to the user query using the provided web search results.
    1. Organize the information logically.
    2. Cite the sources/URLs used.
    3. Answer in the same language as the user query.
    """,
...
def create_agents() -> Dict[str, LlmAgent]:
    ...
    return {
        ...
        "internet_advisor": LlmAgent(model=model_high, name="internet_advisor", instruction=PROMPTS["internet_advisor_system"]),
    }
...
class BioinformaticsPipeline(BaseAgent):
    ...
    async def _run_async_impl(self, context: CallbackContext):
        save_log("--------------new attempts--------------")
        # 입력 데이터 추출 (Payload와 History 분리)
        raw_input = context.user_content
        chat_history = []
        user_query = ""

        # JSON 문자열 또는 딕셔너리 처리
        text_input = ""
        if hasattr(raw_input, 'parts'): 
            text_input = raw_input.parts[0].text
        ...
        try:
            # JSON 파싱 시도 (pipeline.py에서 JSON으로 보냄)
            import json
        ...
        # 히스토리 포맷팅 (Router와 Literature 에이전트에게 맥락 제공)
        history_str = ""
        if chat_history:
            # 최근 5개의 대화만 요약하여 제공
        ...
        prompt_with_context = f"{history_str}\n[Current User Query]: {user_query}"
        ...
        # 1. Router Agent
        # ------------------------------------------------------------------
        # 'internet' 분류를 더 신중하게 하도록 수정
        router_prompt = f"""
        Analyze the conversation history and the user's latest query.
        Latest Query: {user_query}
        History: {history_str if history_str else "None"}
        
        Classify as 'experimenter', 'analyst', or 'internet'.
        - 'experimenter': Wet lab, molecular biology protocols, experiment setup (e.g., phospho-detection, western blot, TiO2).
        - 'analyst': Bioinformatics tools, data analysis pipelines, programming (e.g., R, Python, MaxQuant).
        - 'internet': General follow-up questions about current events, people, or if the user asks for information outside the specific scope of proteomics/bioinformatics research papers.
        
        ONLY output one word.
        """
        role_text = await self._invoke_sub_agent("router", router_prompt)
        role_text = role_text.lower()
        
        if "internet" in role_text:
            role = "internet"
        elif "experimenter" in role_text:
            role = "experimenter"
        else:
            role = "analyst"
        ...
        # ------------------------------------------------------------------
        # Internet Search Pipeline (Short-circuit)
        # ------------------------------------------------------------------
        if role == "internet":
            t_start = time.perf_counter()
            print("Running Internet Search Pipeline...")
            
            # Use query generator (Literature agent) to optimize search query
            search_query = await self._invoke_sub_agent("literature", f"Generate a search engine query for: {user_query}")
            print(f"Internet Search Query: {search_query}")
            
            # Tools call
            search_results = tools.search_internet(search_query)
            
            # Advisor call
            internet_prompt = PROMPTS["internet_advisor_task"].format(
                user_query=user_query,
                search_results=search_results
            )
            final_output_text = await self._invoke_sub_agent("internet_advisor", internet_prompt)
            save_log(f"⏱️ [Time] Internet Pipeline: {time.perf_counter() - t_start:.2f}s\n")
        ...
        contract_text = await self._invoke_sub_agent("contract_builder", f"Analyze: {user_query}")
        ...
        while current_retry < MAX_RETRIES:
            ...
            t_start = time.perf_counter()
            search_query = await self._invoke_sub_agent("literature", f"{prompt_with_context}\nGenerate ONE optimization search query.")
            ...
            # F. Explainer Agent
            t_start = time.perf_counter()
            exp_prompt = PROMPTS["explainer_task"].format(target_role=role, advisor_output=raw_advice, user_query=user_query)
